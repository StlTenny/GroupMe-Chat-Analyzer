...
def create_tables():
    database.connect()
    database.create_tables([GroupChat, ChatMessage, FavoritedMessage, ChatUser, ChatUserName, ChatMention])
    #pdb.set_trace()
...
def create_chat_in_db(chat_ID):
    try:
        with database.atomic():
            chat = GroupChat.create(group_id = chat_ID)
    except Exception as e:
        print("Unable to create group chat in DB")
        print(e)

def populate_database(json, url, self_id, chat_type, chat_ID,
                   msg_count, msg_limit):

    database.connect()
    create_chat_in_db(chat_ID)
    ...
    while msg_count > 0:
        ...
        for i in range(msg_limit):
            ...
            message_id = json['response'][msg][i]['id']
            name = json['response'][msg][i]['name']
            text = json['response'][msg][i]['text']
            attachments = json['response'][msg][i]['attachments']
            favorited_by = json['response'][msg][i]['favorited_by']
            if text: text = text.encode('unicode-escape')  # escape \n, etc.
            
            try:
                user_id = int(user_id)
            except Exception as e:
                print("%s is not a valid user id" % user_id)
                if user_id == 'system':
                    user_id = -1
                elif user_id == 'calendar':
                    user_id = -2
                else:
                    user_id = 0
            ...
            try:
                with database.atomic() as nested_txn:
                    chat_user_name = ChatUserName.create(
                        group_id = chat_ID,
                        user_id = user_id,
                        user_name = name,
                        last_used = epoch_time)
            except Exception as e:
                print("skipping username creation")
            try:
                with database.atomic() as nested_txn:
                ...
                print("Unable to create chat message")
                print(e)
            if favorited_by:
                try:
                    with database.atomic() as nexted_txn:
                        for favorite in favorited_by:
                            fav_message = FavoritedMessage.create(
                                group_id = chat_ID,
                                message_id = message_id,
                                user_id = user_id,
                                favorite_id = favorite)
                except Exception as e:
                    print("Unable to create favorite")
                    print(e)
            if attachments:
                for attachment in attachments:
                    if attachment['type'] == 'mentions':
                        for mention_target in attachment['user_ids']:
                            try:
                                with database.atomic():
                                    mention = ChatMention.create(
                                            group_id = chat_ID,
                                            message_id = message_id,
                                            user_id = user_id,
                                            target_id = int(mention_target),
                                            created_at = epoch_time)
                            except Exception as e:
                                print("unable to create mention")
                                print(e)

            #except Exception as e:
            #    if not str(e) == "UNIQUE constraint failed: chatuser.group_id, chatuser.user_id":
            #        pdb.set_trace()
            #        print(e)
        ...
        if msg_count == 0:
            database.close()
            # In the case we need to cleanup 
...
def get_chat_info(token):
    chat_ID = '43893576'
    token = 'a4f5HHl1MuKoLY7G6EIyY1tUjlaew3bg0OilDnXB'
    chat_type = 'group'
...
def get_chat(token, chat_type, chat_ID):
    ...
    populate_database(i_json, url, self_id, chat_type, chat_ID,
